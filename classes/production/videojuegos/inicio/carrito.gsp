...
<script>
    $(document).ready(function() {
        ...
        var cartCount = $('#cart-count'); // Elemento del carrito

        // Función para mostrar un mensaje si el carrito está vacío
        function mostrarMensajeCarritoVacio() {
            if (cartItemsContainer.children().length === 0) {
                var emptyCartMessage = $('<p>').text('El carrito está vacío... ¡Puedes comprar videojuegos en la seccion de Categorias!');
        ...
        function mostrarElementosCarrito() {
            ...
            var totalPrice = 0;
            cartItemsContainer.empty(); // Limpia los elementos previos del carrito
            var cartItemCount = 0; // Inicializa el contador del carrito

            $.each(carrito, function(index, item) {
                var listItem = $('<li>');
                var itemName = $('<span>').text(item.title);
                var itemPrice = $('<span>').text('$' + (item.price * (item.quantity || 1)).toFixed(2)); // Calcula el precio total
                var itemQuantity = $('<span>').text('x ' + (item.quantity || 1)); // Muestra la cantidad
                ...
                increaseButton.on('click', function() {
                    ...
                    if (existingItem) {
                        ...
                        cartItemCount++; // Incrementa el contador del carrito
                    }

                    // Actualiza el carrito en el almacenamiento local
                    localStorage.setItem('cart', JSON.stringify(carrito));
                    actualizarContadorCarrito(cartItemCount); // Actualiza el contador del carrito en la vista
                    mostrarElementosCarrito(); // Vuelve a mostrar los elementos del carrito
                });

                decreaseButton.on('click', function() {
                    if (item.quantity && item.quantity > 1) {
                        item.quantity -= 1;
                        cartItemCount--; // Decrementa el contador del carrito
                    } else {
                ...
                deleteButton.on('click', function() {
                    ...
                    carrito.splice(index, 1);
                    cartItemCount -= item.quantity || 1; // Decrementa el contador del carrito
                    // Actualiza el carrito en el almacenamiento local
                    localStorage.setItem('cart', JSON.stringify(carrito));
                    actualizarContadorCarrito(cartItemCount); // Actualiza el contador del carrito en la vista
                    mostrarElementosCarrito(); // Vuelve a mostrar los elementos del carrito
                ...
                cartItemCount += item.quantity || 1; // Incrementa el contador del carrito
            });
        ...
        // Función para actualizar el contador del carrito en la vista y en el almacenamiento local
        function actualizarContadorCarrito(count) {
            cartCount.text(count); // Actualiza el contador del carrito en la vista
            localStorage.setItem('cartItemCount', count); // Actualiza el contador del carrito en el almacenamiento local
        }
...
<script>
    $('#comprar-button').on('click', function() {
        ...
        var cedula = $('#cedula').val(); //Recuperamos el valor de la cedula correspondiente del modal
        ...
        $.ajax({
            ...
            success: function(clienteId) {
                ...
                cart.forEach(function(item) {
                    // Creamos un objeto json con los datos del pago
                    var pagoData = {
